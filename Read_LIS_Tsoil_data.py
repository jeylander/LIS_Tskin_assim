...
def Read_LIS_Tsoil_data(inDTG_YYYY, inDTG_mm, inDTG_dd, inDTG_hh, the_lat_point, the_lon_point, filedir):
    ...
    for x in range (0, numcols-1):
        temp_array_lon[x]=np.nanmax(lon_data[:,x])

    for y in range (0, numrows-1):
        temp_array_lat[y]=np.nanmax(lat_data[y,:])
